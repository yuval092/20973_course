...
from src.config import BOARD_CENTER, SQUARE_SIZE, TABLE_HEIGHT, TILT_THRESHOLD_COS, DISPLACEMENT_THRESHOLD
...
class BoardObserver:
    ...
    @staticmethod
    def _is_ignored_body(name: str) -> bool:
    ...
    def verify_stability(self):
        ...
        for i in range(self.model.nbody):
            ...
            if up_z < TILT_THRESHOLD_COS:
                raise StabilityError(f"Piece '{name}' has tipped over (up_z={up_z:.3f})")

            pos = self.data.body(i).xpos
            if pos[2] < TABLE_HEIGHT - 0.01:
                raise StabilityError(f"Piece '{name}' is below the table level (z={pos[2]:.3f})")
            ...
            if dx < 0.35 and dy < 0.35:
                if pos[2] < TABLE_HEIGHT + 0.05:
                    ...
                    err_m = np.sqrt((dist_x * SQUARE_SIZE) ** 2 + (dist_y * SQUARE_SIZE) ** 2)

                    if err_m > SQUARE_SIZE * DISPLACEMENT_THRESHOLD:
                        raise StabilityError(f"Piece '{name}' is significantly displaced from square center (err={err_m:.3f}m)")
